    ...
    <dialog ref={dialogRef} aria-labelledby={titleId} onCancel={(event) => { event.preventDefault(); onClose(); }}
      ...
      <div className="flex max-h-[92dvh] flex-col">
        <header className="flex shrink-0 items-center justify-between gap-4 border-b border-border px-5 py-3">
          ...
          <button type="button" autoFocus onClick={onClose} aria-label="Cerrar detalle de solicitud" className="rounded-lg p-2 text-muted-foreground hover:bg-muted focus-visible:outline-2 focus-visible:outline-ring"><X className="h-5 w-5" aria-hidden="true" /></button>
        </header>
        <div className="grid min-h-0 overflow-y-auto lg:grid-cols-[minmax(0,1.3fr)_minmax(320px,1fr)]">
          <div className="min-w-0 p-5 sm:p-8">
            ...
            <section className="mt-8" aria-label="Descripción de la solicitud">
