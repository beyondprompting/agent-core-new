...
export function RequestCommentsSection({ taskId }: { taskId: Id<"tasks"> }) {
  ...
  return <aside aria-labelledby="request-comments-title" className="border-t border-border bg-muted/40 p-5 sm:p-6 lg:border-l lg:border-t-0">
    <h3 id="request-comments-title" className="mb-5 flex items-center gap-2 text-sm font-semibold"><MessageSquare className="h-4 w-4" />Comentarios y actividad</h3>
    <form onSubmit={async e => { e.preventDefault(); if (await submission.save(text, files)) { setText(""); setFiles([]); } }} className="rounded-xl border border-border bg-card p-3">
      <label htmlFor="request-comment" className="sr-only">Escribir un comentario</label>
      <textarea id="request-comment" value={text} onChange={e => setText(e.target.value)} disabled={submission.busy} maxLength={10000} placeholder="Escribí un comentario…" rows={4} className="w-full resize-y rounded bg-transparent text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring" />
      <div className="mt-3 border-t border-border pt-3"><TaskPanelFilePicker files={files} onChange={setFiles} disabled={submission.busy} /></div>
      <Button type="submit" size="sm" className="mt-3" disabled={submission.busy || !text.trim()}>{submission.busy && <Loader2 className="h-4 w-4 animate-spin" />}Comentar</Button>
